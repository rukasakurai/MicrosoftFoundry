...
- **Entra ID Governance** — least-privilege scoping, **PIM** (just-in-time elevation, no standing
  privilege), **access packages / entitlement management** (approval-gated, time-bound), **access
  reviews** (recertify/retire access), **lifecycle workflows** (retire agents → no orphaned credentials),
  **managed identities / workload identity federation** (secret-less auth), and sign-in/audit logs.

**What it does NOT do (pair accordingly):**
- **No in-context AI safety** — it can't stop prompt injection, jailbreak, hallucination, or memory
  poisoning at the reasoning layer (→ Foundry guardrails / evaluations).
...
| LLM02 Sensitive Info Disclosure | Supporting (material) | OBO/least-privilege scoping limits what data an agent can reach | Purview classifies/DLP; Foundry PII guardrail redacts |
| LLM03 Supply Chain | Supporting (material) | Verifiable agent identity + ownership/lifecycle ("who deployed this agent") | Defender/GHAS (vuln), API Center (tool/MCP registry), Purview (model/data lineage) |
...
| LLM05 Improper Output Handling | Not applicable | (least-privilege downstream identity only limits blast radius) | App code / secure SDLC / GHAS / Foundry output guardrails own it |
| LLM06 Excessive Agency | **Primary (co)** | Least-privilege scopes, OBO consent, **PIM JIT**, access packages, access reviews | Foundry constrains in-context behavior (task adherence, HITL); APIM throttles action rate |
| LLM07 System Prompt Leakage | Supporting (material) | **Managed identity / workload-identity federation** ⇒ no secrets in the prompt; enforce authZ *outside* the LLM | Foundry/architecture prevent extraction; Key Vault stores secrets |
...
| LLM10 Unbounded Consumption | Supporting (weak) | Auth gate (Conditional Access blocks unauthenticated/high-risk callers) + per-caller attribution | APIM GenAI gateway / Foundry Quota / Azure Cost Management enforce limits |
...
| ASI03 Agent Identity & Privilege Abuse | **Primary** | Unique non-forgeable agent identity, **PIM JIT**, access reviews, lifecycle retirement (no orphaned creds), risk-based block | Sentinel/Defender UEBA add detection; Foundry covers in-context behavior only |
...
| ASI06 Memory & Context Poisoning | Supporting (weak) | Identity-scoped write access to the memory store; OBO per-user partitioning reduces cross-user contamination | Foundry memory integrity + content guardrails + Purview validate content |
...
**Tally:** Primary 5 (LLM06, LLM08, ASI03, ASI07, ASI10 — several co-owned) · Supporting (material) 6
(LLM02, LLM03, LLM07, ASI01, ASI02, ASI04) · Supporting (weak) 7 (LLM01, LLM04, LLM10, ASI05, ASI06,
ASI08, ASI09) · Not applicable 2 (LLM05, LLM09).
...
- **ASI03 — Agent Identity & Privilege Abuse:** the single risk Entra Agent ID was built for. A unique,
  non-forgeable identity per agent + PIM just-in-time elevation + access reviews + lifecycle retirement
  directly attack impersonation, confused-deputy, standing over-privilege, and orphaned credentials.
- **LLM06 — Excessive Agency:** least-privilege scopes, on-behalf-of consent, PIM, and approval-gated,
  time-bound access packages cap an agent's standing authority at the identity layer — the core OWASP
...
For content-, model-, and runtime-centric risks (LLM01, LLM04, LLM05, LLM09, ASI05, ASI06, ASI08, ASI09),
...
- **Secret-less auth** — managed identities / workload-identity federation remove the *need* to embed
  credentials in prompts or code (LLM07).
