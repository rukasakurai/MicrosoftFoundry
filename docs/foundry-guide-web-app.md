...
When the APIM token-usage sample is enabled in the same azd environment, the two
apps share its App Service plan instead of allocating another worker:
...
### 2026-07-24 11:13 JST
- Result: PASS with fixes
- Platform/Context: WSL2, isolated `azd` environment, Playwright MCP
- Notes:
  - Provisioned the quota table, shared the existing token-sample App Service plan, and verified `401` without auth plus only Foundry Agent Consumer and Storage Table Data Contributor on the app identity.
  - A four-request concurrent load accepted three reservations and rejected one; the ledger increased by exactly the 1,200 charged tokens after follow-up drain calls and retained zero reserved tokens.
  - A two-turn continuation completed without errors and updated the same authoritative usage view after both responses.
  - Desktop and 390x844 mobile usage, feedback, responsive layout, and the final 406-token-reservation-versus-350-token-remaining message passed without horizontal overflow.
  - Existing simple, APIM-only, and strict regressions remained exact at 54/54, 54/54, and 36/36 measured/reported tokens.
