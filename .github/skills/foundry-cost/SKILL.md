...
| Red-team tests / evaluations | Target and judge model calls; managed attacker/evaluator usage when used | Direct PyRIT has no service meter; the current campaign bills Foundry Models tokens. Managed Evaluation SDK or `AI Red Teaming Agent` paths can add Foundry Tools meters. |
...
- [pr-diff-cost-review.md](references/pr-diff-cost-review.md) — review marginal cost
  impact in a PR.
- [workload-assumptions.md](references/workload-assumptions.md) — clarify the
  workload inputs required before a dollar estimate is meaningful.
...
- Red-team billing depends on the engine: count target and judge model tokens for
  direct PyRIT, and check for additional evaluation meters on managed paths.
