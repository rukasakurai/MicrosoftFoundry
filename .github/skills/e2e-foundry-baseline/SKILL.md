...
| 5 | Agent creation (.NET) `scripts/dotnet/CreateAgent` | `dotnet run` → agent created | ~17s (warm; first run adds restore+build) | ✅ builds and creates an agent. Uses `AzureCliCredential` (not `DefaultAzureCredential`, which stalls ~3 min probing IMDS locally). SDK is prerelease-only (`Azure.AI.Projects[.Agents]` 2.1.0-beta.x) |
