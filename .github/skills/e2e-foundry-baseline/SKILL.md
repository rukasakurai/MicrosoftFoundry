...
| 6 | MCP agent + evidence-safe classification (`scripts/classify-agent-run.sh`) | create + run; classify output items as **pass/fail/invalid** (not just prose). `mcp_call` with output → `pass`; message-only → `invalid` | ~15s | ✅ easy with an auth-free MCP. `pass` + `invalid` are testable here; the `fail`/consent branch needs an OAuth connection (flow 11). `create-mcp-agent.sh` runs the classifier automatically |
