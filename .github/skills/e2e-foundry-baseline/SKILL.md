...
| `docs/policies.md` | docs-accuracy check (no provisioning flow) | ~125s | ✅ verify claims against Microsoft Learn, the live **Operate → Compliance → Policies** portal (Preview badge + "Create policy" wizard footer) with Playwright, and `az policy definition list` (guardrail defs are `[Preview]`, `Audit`-only; `approved models` is GA + `Deny`); **preview**, so confirm caveats still hold and date the result |
